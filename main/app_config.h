...
/* Frequency reset sweep: the gateway visits every preset and sends
 * Config(FREQUENCY=preset[0]) on each until a node acknowledges. The node is
 * then walked through the normal two-stage change to preset[0]. */
#define APP_FREQUENCY_RESET_TARGET_INDEX 0U
#define APP_FREQUENCY_RESET_PROBE_RETRIES 2U
...
// Give up the whole request when no ImageCmdAck/ImageStart arrives within this
// window. Without it an offline node keeps the gateway in image_busy() forever
// (page keys are locked while busy, so the user could not cancel).
#define APP_IMAGE_REQ_TIMEOUT_MS 5000U
...
/* Split-speed SPI: init and CASET/RASET/RAMWR go out on a 10 MHz panel IO,
 * RGB565 pixel DMA on a separate 16 MHz device. GPSPI offers 80/5 = 16 MHz or
 * 80/4 = 20 MHz here; 16 MHz is the stable choice on the DK01 LCD link.
 * duty_cycle_pos = 64/256 widens the MOSI setup window before the panel's
 * rising-edge sample. */
...
/* GPIO drive strength 0..3. The default level 2 corrupts the picture above
 * ~11 MHz on this link; level 3 is clean at 16 MHz. */
